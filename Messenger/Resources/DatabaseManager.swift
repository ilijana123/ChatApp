...
//        }
    ...
    func getUserActivityStatus(email: String, completion: @escaping (Bool?) -> Void) {
        let safeEmail = DatabaseManager.safeEmail(emailAddress: email)
    ...
    func updateUserActivityStatus(email: String, isActive: Bool, completion: @escaping (Bool) -> Void) {
        let safeEmail = DatabaseManager.safeEmail(emailAddress: email)
        let ref = Database.database().reference().child("users").child(safeEmail)

        ref.updateChildValues(["isActive": isActive]) { error, _ in
            if let error = error {
